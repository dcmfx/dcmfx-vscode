 ...
 */
export function new_write_context(config) {
  return new P10WriteContext(
    $option.unwrap(config, $p10_write_config.new$()),
    toList([]),
    0,
    false,
...
export function data_set_to_tokens(
  ...
  let remove_fmi_transform = $p10_filter_transform.new$(
    (tag, _, _1, path) => {
      return !$data_set_path.is_root(path) || !$data_element_tag.is_file_meta_information(
        tag,
      );
    },
  );
...
function prepare_file_meta_information_token_data_set(
  ...
  if ($ instanceof Ok) {
    file_meta_information_version = $[0];
  } else {
    throw makeError(
      "let_assert",
      FILEPATH,
      ...
      if (token instanceof $p10_token.FileMetaInformation) {
        ...
        return $result.try$(
          ...
          (new_transfer_syntax) => {
            ...
            let new_context = new P10WriteContext(
              context.config,
              context.p10_bytes,
              context.p10_total_byte_count,
              context.is_ended,
            ...
            return $result.map(
              ...
              (token_bytes) => {
                return new P10WriteContext(
                  ...
                  new_context.is_ended,
                  new_context.transfer_syntax,
                  new_context.zlib_stream,
        ...
        if ($ instanceof Some) {
          ...
          let data = _block;
          let _pipe$2 = new P10WriteContext(
            context.config,
