...
function* doConvertJsonToP10(src: vscode.Uri, dst: vscode.Uri) {
  ...
  const tag = dictionary.Item$Item$tag(dictionary.transfer_syntax_uid);
  if (!data_set.has(dataSet, tag)) {
    const uidList = arrayToGleamList([
      transfer_syntax.TransferSyntax$TransferSyntax$uid(
